 ...
 * by the max call depth (\ref UBPF_MAX_CALL_DEPTH) must be
 * less than or equal to the total stack size for the eBPF VM
 * (\ref UBPF_EBPF_STACK_SIZE).
 */
#if !defined(UBPF_EBPF_STACK_SIZE)
#define UBPF_EBPF_STACK_SIZE (UBPF_MAX_CALL_DEPTH * UBPF_EBPF_LOCAL_FUNCTION_STACK_SIZE)
#endif
...
#define UBPF_EBPF_NONVOLATILE_SIZE (sizeof(uint64_t) * 5)
    ...
    /**
     * @brief Set a load/store pointer mask for JIT compilation.
     *
     * When configured, JIT-compiled load and store addresses are transformed as
     * <tt>(address & mask) + offset</tt> before the native memory access.
     *
     * @param[in] vm The VM.
     * @param[in] mask Pointer mask.
